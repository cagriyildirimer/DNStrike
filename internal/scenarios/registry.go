...
func DefaultRegistry() *Registry {
	...
	definitions := []models.ScenarioMetadata{
		...
		{ID: "dns-tunneling", Name: "DNS Tunneling & Exfiltration Audit", Category: "audit", Description: "Encodes synthetic secret payload chunks into high-entropy subdomain labels over TXT/A queries to test DLP/SIEM covert channel detection.", SupportedProtocols: []string{"udp", "tcp"}, RiskLevel: "MEDIUM", DefaultConfig: map[string]any{"domain": "example.com", "payload_bytes": 256}, RecommendedLimits: models.Limits{MaxQPS: 50, MaxDuration: 30, MaxWorkers: 5}},
		{ID: "water-torture", Name: "Water Torture (Random Subdomain Flood)", Category: "volume", Description: "Sends pseudo-random non-existent subdomains at high QPS to bypass resolver caches and stress upstream authoritative servers.", SupportedProtocols: []string{"udp", "tcp"}, RiskLevel: "HIGH", DefaultConfig: map[string]any{"domain": "example.com", "qps": 200, "duration": 15, "workers": 20}, RecommendedLimits: limit},
		{ID: "ecs-manipulation", Name: "EDNS0 Client Subnet (ECS) Geo-Spoofing", Category: "audit", Description: "Injects custom EDNS0 client subnets (US, EU, JP, BR) into DNS queries to evaluate Geo-DNS routing and ECS privacy posture.", SupportedProtocols: []string{"udp", "tcp"}, RiskLevel: "LOW", DefaultConfig: map[string]any{"domain": "example.com"}, RecommendedLimits: models.Limits{MaxQPS: 20, MaxDuration: 30, MaxWorkers: 2}},
		{ID: "dnssec-audit", Name: "DNSSEC Cryptographic & Health Audit", Category: "audit", Description: "Queries DNSKEY, DS, and RRSIG records with DNSSEC OK (DO) bit to audit cryptographic algorithms, expiration dates, and validation posture.", SupportedProtocols: []string{"udp", "tcp"}, RiskLevel: "LOW", DefaultConfig: map[string]any{"domain": "example.com"}, RecommendedLimits: models.Limits{MaxQPS: 20, MaxDuration: 30, MaxWorkers: 2}},
